...
use crate::xurvm::var::{Enviroment, Value, CallableHanderF};
...
impl XurVM {
    ...
    pub fn add_fn(&mut self, name: &str, arg_n: usize, f: CallableHanderF) {
        self.global_state.insert(name.into(), Value::create_fn(name, arg_n, f));
    }
    ...
    fn add_builtins(mut self) -> Self {
        ...
        self.add_fn("__get_symbol_bind__", 1, |env, args| {
            ...
            let binding = env.get_value(value_1.as_ref())
                .context(format!("Failed to get binding for \"{}\"", &value_1))?
                .clone();

            let fn_ref = binding.get_callable()
                .context(format!("Faiiled to convert \"{}\" bind to callable.",  &value_1))?;

            return Ok(Value::Function(fn_ref));
            Ok(Value::create_fn(&format!("__sym_bind_@{}", value_1), 1, |env, args| {
                let mut args = env.resolve_args(args)?;
                let value_1 = args.pop().context("Failed to pop arg1")?.get_string()?;
    ...
    fn get_value(&mut self, ex: Expression) -> anyhow::Result<Value> {
        ...
        match ex {
            Expression::Variable(s) => {
                let v = self.global_state.get(&s).context(format!("\"{}\" is not defined.", s))?;
                Ok(v.clone())
            },
